...
//		pagerTitleStrip.setBackgroundColor(getResources().getColor(android.R.color.holo_blue_dark));
		...
		m_vp.setOnPageChangeListener(new MyOnPageChangeListener());
		  ...
		  AnsyTry anys=new AnsyTry(map,dialogLoad,mfragment1);
		  anys.execute();
		  
		  //广播接收器的动态注册，Action必须与Service中的Action一致
	      registerReceiver(br, new IntentFilter("ACTION_MY"));
	      //启动检测版本更新service轮询
	      PollingUtils.startPollingService(testActivity.this,60*60*24*2, UpdatePollingService.class, UpdatePollingService.ACTION_UPDATE);
	}
...
//		anys.execute();		
		...
		//启动轮询service
		 new Handler().postDelayed(new Runnable(){  
		     public void run() {  
		    	 //启动轮询service
			     // PollingUtils.startPollingService(testActivity.this, 15, PollingService.class, PollingService.ACTION);
		     }  
		}, 500);
	}
    ...
    private void setParams(LayoutParams lay) {  
	    ...
	    class AnsyTry extends AsyncTask<String, HashMap<String, String>, List<Map<String, Object>>>{
	          ...
	          public AnsyTry(HashMap<String, String> map,Dialog dialogLoad, fragment1 mfragment1) {
	              super();
	              this.map = map;
...
//				    			pBar = new ProgressDialog(testActivity.this);
//								pBar.setTitle("正在下载");
//								pBar.setMessage("请稍候...");
